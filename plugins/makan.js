    ...
    if (/makan|eat/i.test(command)) {
      ...
      if (args[0] === 'ayamg') {
        if (global.DATABASE._data.users[m.sender].stamina < 100) {
          ...
          } else conn.reply(m.chat, `Ayam goreng mu kurang weh` ,m)
        } else conn.reply( m.chat, `Wes wareg weh`, m)
      } else if (args[0] === 'ayamb') {
        if (global.DATABASE._data.users[m.sender].stamina < 100) {
          if (global.DATABASE._data.users[m.sender].ayamb >= count * 1) {
            global.DATABASE._data.users[m.sender].ayamb -= count * 1
            global.DATABASE._data.users[m.sender].stamina += spertamina * count
            conn.reply(m.chat, `Nyam nyam`, m)
          } else conn.reply(m.chat, `Ayam bakar mu kurang weh` ,m)
        } else conn.reply( m.chat, `Wes wareg weh`, m)
      } else if (args[0] === 'leleb') {
        if (global.DATABASE._data.users[m.sender].stamina < 100) {
