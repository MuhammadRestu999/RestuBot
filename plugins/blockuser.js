let handler = async(m, { conn, args }) => {
    ...
    if (mention.length == 0) throw 'Tag salah satu lah'
    conn.blockUser(mention, 'add')
    m.reply("Berhasil memblokir [ @" + mention.split("@")[0] + " ] !")
}
handler.help = ['block <tag/reply>']
handler.tags = ['owner']
handler.command = /^block(user)?$/i
handler.mods = true
