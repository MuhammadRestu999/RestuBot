  ...
  try {
    ...
    if (new Date - global.DATABASE._data.users[m.sender].lastbansos > 300000) {
      if (Aku > Kamu) {
        ...
        conn.sendButton( m.chat, `Sorry Gan Lu g Berhasil Korupsi bansos Dan Tidak masuk penjara karna kamu *melarikan diri🏃*`, `RestuBot`, `Kembali`, `${usedPrefix}menu`, m)
        global.DATABASE.data.users[m.sender].lastbansos = new Date * 1
      }
    } else conn.sendButton(m.chat, `Kamu sudah Melakukan Korupsi Bansos, dan kamu harus menunggu selama ${timers} agar bisa korupsi bansos kembali`, `RestuBot`, `Menu`, `${usedPrefix}menu`, m)
  } catch (e) {
    throw `${e}`
  }
}
