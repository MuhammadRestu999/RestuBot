...
let handler = async (m, { conn, usedPrefix, owner }) => { 
   ...
   //                  
        ...
        global.DATABASE._data.users[m.sender].lasfishing == undefined ? global.DATABASE._data.users[m.sender].lasfishing = new Date : global.DATABASE._data.users[m.sender].lasfishing = global.DATABASE._data.users[m.sender].lasfishing
        //
         if (psepick > 0 ) {
        ...
        global.DATABASE._data.users[m.sender].nila += nila * 1
         ...
         global.DATABASE._data.users[m.sender].lastfishing = new Date * 1
           ...
           } else conn.sendButton( m.chat, `Kamu tidak punya kail pancing🎣`, `Games Wa bot`, `🛒beli kail pancing`, '+beli pancing')
           } catch (e) {
        console.log(e)
