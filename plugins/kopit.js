...
const handler = async (m, { conn, args, command, usedPrefix }) => {
    const reply = (sket) => {
        conn.sendMessage(m.chat, sket, MessageType.text, {
            quoted: m
        })
    }
    ...
    try {
        ...
        let kraw = await axios(linko)
        kraw = kraw.data
        let kres = kraw // JSON.parse(kraw)

        let rraw = await axios("https://api.kawalcorona.com/")
        rraw = rraw.data
        let rres = rraw // JSON.parse(rraw)
        ...
        let image = await Jimp.read(path.join(__dirname, "..", "/lib/", "covid.jpg"))
        //Load File
        ...
        conn.sendMessage(m.chat, image, MessageType.image, {
