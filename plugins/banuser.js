let handler = async (m, { conn, args }) => {
  if(!args || !args[0]) return m.reply("Siapa yang mau di banned om?")
  let mention = m.mentionedJid[0] || conn.parseMention(args[0]) || (args[0].replace(/[@.+-]/g, "").replace(" ", "") + "@s.whatsapp.net") || ""
  if(!mention) return m.reply("Tag salah satu lah")
  if(!(mention in global.DATABASE._data.users)) return m.reply("User tidak terdaftar dalam DATABASE!!")
  let user = global.DATABASE._data.users[mention]
  if(user.Banneduser) return m.reply("User telah terbanned!!")
